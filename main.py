...
return (
  user_id == OWNER_ID or user_id in SUDO_USERS or user_id in AUTH_CHANNELS # ✅ Checks if user_id matches any channel ID
)
...
await message.reply_text(f"**⚠️ User {
  target_user_id
} is already in the sudo list.**")
elif action == "remove":
...
await m.reply_text(f"⚠️ An error occurred: {
  str(e)}")
...
logging.error(f"Error retrieving videos: {
  e
}")
return None, None
...
await editable.edit(f"`𝗧𝗼𝘁𝗮𝗹 🔗 𝗟𝗶𝗻𝗸𝘀 𝗙𝗼𝘂𝗻𝗱 𝗔𝗿𝗲 {
  len(links)}\n\n🔹Img : {
  img_count
}  🔹Pdf : {
  pdf_count
...
url = f"https://dragoapi.vercel.app/video/ {
  url
}"

elif 'videos.classplusapp' in url:
url = requests.get(f'https://api.classplusapp.com/cams/uploader/video/jw-signed-url?url= {
  url
}', headers = {
...
headers = {
  'Host': 'api.classplusapp.com', 'x-access-token': 'eyJjb3Vyc2VJZCI6IjQ1NjY4NyIsInR1dG9ySWQiOm51bGwsIm9yZ0lkIjo0ODA2MTksImNhdGVnb3J5SWQiOm51bGx9', 'user-agent': 'Mobile-Android', 'app-version': '1.4.37.1', 'api-version': '18', 'device-id': '5d0d17ac8b3c9f51', 'device-details': '2848b866799971ca_2848b8667a33216c_SDK-30', 'accept-encoding': 'gzip'
}
params = (('url', f' {
  url
}'),)
...
id = url.split("/")[-2]
url = f"https://anonymouspwplayer-554b25895c1a.herokuapp.com/pw?url= {
  url
}&token= {
  raw_text4
}"
...
url = f"https://kgs-v4.akamaized.net/kgs-cv/ {
  part3
}/ {
  part4
}/ {
...
cmd = f'yt-dlp --add-header "referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f " {
  ytf
}" " {
  url
}" -o " {
...
cmd = f'yt-dlp --cookies youtube_cookies.txt -f " {
  ytf
}" " {
  url
}" -o " {
...
cpvod = f'**[🎬] 𝗩𝗶𝗱_𝗜𝗱 : {
  ...
  url
}">__Click Here to Watch Video__</a>\n\n📚𝗕𝗮𝘁𝗰𝗵 𝗡𝗮𝗺𝗲 ➤ {
  b_name
}\n\n📥 𝗘𝘅𝘁𝗿𝗮𝗰𝘁𝗲𝗱 𝗕𝘆 ➤ {
  CR
}**'
...
await asyncio.sleep(4)
copy = await bot.send_document(chat_id = m.chat.id, document = f' {
  name
}.pdf', caption = cc1)
count += 1
...
with open(f' {
  name
}.jpg', 'wb') as file: # Save as JPG (or PNG if you want)
file.write(response.content)

# Send the image document
await asyncio.sleep(2) # Non-blocking sleep
copy = await bot.send_photo(chat_id = m.chat.id, photo = f' {
  name
}.jpg', caption = cimg)
count += 1
...
f'🔗𝗨𝗿𝗹 » <a href=" {
  url
}">__**Click Here to See Link**__</a>`')
...
f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
f"✅𝗦𝗧𝗔𝗧𝗨𝗦 » 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘𝗗`")
await m.reply_text(f"<pre><code>📥𝗘𝘅𝘁𝗿𝗮𝗰𝘁𝗲𝗱 𝗕𝘆 ➤『 {
  CR
}』</code></pre>")
